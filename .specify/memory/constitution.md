...
- Auth/session: Starlette `SessionMiddleware` uses cookie sessions; all client fetches include `credentials: 'include'`. Backend routes must reject unauthenticated access.
...
- Default data: `ensure_dev_user` seeds the local dev account on startup; do not remove without replacing local auth flow.
