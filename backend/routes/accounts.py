...
from models import SocialAccount, SocialAccountBase, Researcher
...
def get_accounts(
    ...
    accounts = session.exec(select(SocialAccount).where(SocialAccount.researcher_id == current_user.id)).all()
    return accounts
...
def create_account(
    ...
    existing = session.exec(select(SocialAccount).where(SocialAccount.username == account_base.username)).first()
    if existing:
