...
def _create_researcher(session: Session, email_suffix: str = None) -> Researcher:
    ...
    researcher = Researcher(name="Results", lastname="Test", email=email, password="hash")
...
def test_get_results_summary_empty(client: TestClient, session: Session, auth_headers):
    """Test getting results for an experiment with no participants."""
    researcher = _create_researcher(session, "empty")
    project = _create_project(session, researcher.id)
    experiment = _create_experiment(session, project.id)

    # Request results
    response = client.get(f"/api/experiments/{experiment.id}/results", headers=auth_headers)
...
def test_get_results_summary_with_participants(client: TestClient, session: Session, auth_headers):
    """Test getting results summary with participant sessions."""
    researcher = _create_researcher(session, "summary")
    project = _create_project(session, researcher.id)
    experiment = _create_experiment(session, project.id)
...
def test_export_csv_format(client: TestClient, session: Session, auth_headers):
    """Test CSV export with proper headers and formatting."""
    researcher = _create_researcher(session, "csv")
    project = _create_project(session, researcher.id)
    experiment = _create_experiment(session, project.id)
    account = _create_social_account(session, researcher.id)
    video = _create_video(session, experiment.id, account.id)
...
def test_export_csv_selected_participants(client: TestClient, session: Session, auth_headers):
    """Test CSV export with participant ID filtering."""
    researcher = _create_researcher(session, "csv_filter")
    project = _create_project(session, researcher.id)
    experiment = _create_experiment(session, project.id)
    account = _create_social_account(session, researcher.id)
    video = _create_video(session, experiment.id, account.id)
...
def test_export_json_format(client: TestClient, session: Session, auth_headers):
    """Test JSON export with full interaction details."""
    researcher = _create_researcher(session, "json")
    project = _create_project(session, researcher.id)
    experiment = _create_experiment(session, project.id)
    account = _create_social_account(session, researcher.id)
    video = _create_video(session, experiment.id, account.id)
...
def test_export_json_selected_participants(client: TestClient, session: Session, auth_headers):
    """Test JSON export with participant filtering."""
    researcher = _create_researcher(session, "json_filter")
    project = _create_project(session, researcher.id)
    experiment = _create_experiment(session, project.id)
    account = _create_social_account(session, researcher.id)
    video = _create_video(session, experiment.id, account.id)
...
def test_export_requires_format(client: TestClient, session: Session, auth_headers):
    """Test that export endpoint requires format parameter."""
    researcher = _create_researcher(session, "format_req")
    project = _create_project(session, researcher.id)
    experiment = _create_experiment(session, project.id)

    # Request without format
    response = client.post(
        f"/api/experiments/{experiment.id}/results/export", headers=auth_headers, json={}
    )
    assert response.status_code == 422  # Validation error


def test_export_invalid_experiment(client: TestClient, auth_headers):
